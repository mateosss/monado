...
oxr_instance_create(struct oxr_logger *log, const XrInstanceCreateInfo *createInfo, struct oxr_instance **out_instance)
{
	...
	/* ---- HACK ---- */
	// XXXANS: When running an OpenXR app with OXR_DEBUG_GUI=1 it crashes
	// ANS: It does so in master as well, but seems to be a problem only for
	// the openxr-simple-example, helloxr works well
...
#undef POPULATE_ROLES
	...
	// XXXANS: This next line is only being executed by the app but not by monado-service,
	...
	// affected
	// ANS: Indeed, asked on discord and is something that will eventually work.
