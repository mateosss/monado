...
xreal_air_camera_create(struct xrt_prober *xp, struct xreal_air_system *sys)
{
	...
	/* Finally, start the video feed. Just use index 0, the mode is a dummy anyway */
	xrt_fs_stream_start(finder.xfs, &cam->in_sink, XRT_FS_CAPTURE_TYPE_TRACKING, 0);
...
receive_cam_frame(struct xrt_frame_sink *sink, struct xrt_frame *xf)
{
	...
	// Push to tracker when we have a matching left/right frame
	if (cam->last_left && cam->last_right &&
	    cam->last_left->source_sequence == cam->last_right->source_sequence) {
		xreal_air_tracker_push_slam_frames(cam->sys->tracker, cam->last_left, cam->last_right);
	}
